...
import type { AskAssistantRequest, AskAssistantResponse, ChatMessage } from '../types';
...
async function verifyAuth(token: string): Promise<AuthContext> {
  try {
    ...
    logError('verifyAuth', error as Error, { tokenPrefix: token.substring(0, 10) });
    throw new AuthError('Token invalide ou expiré', 'Veuillez vous reconnecter');
  }
}
...
async function verifyAppCheck(
  ...
  if (!appCheckToken) {
    ...
    logError('verifyAppCheck', new Error('AppCheck token manquant'), { uid });
    return false;
  }
  ...
  try {
    ...
    logError('verifyAppCheck', error as Error, { uid });
    return false;
  }
}
  ...
  try {
    ...
    if (snapshot.size >= RATE_LIMIT_MAX_REQUESTS) {
      ...
      const retryAfter = Math.ceil(
        (oldestRequest.data().timestamp + RATE_LIMIT_WINDOW_MS - now) / 1000
      );

      throw new AppError(
        ErrorCodes.RATE_LIMITED,
        'Trop de requêtes. Veuillez patienter.',
        `Limite: ${RATE_LIMIT_MAX_REQUESTS} requêtes par minute. Réessayez dans ${retryAfter} seconde(s).`,
        429
      );
    }
    ...
    logError('checkRateLimit', error as Error, { uid });
  }
}
...
function validateRequest(request: AskAssistantRequest): void {
  if (!request.message || typeof request.message !== 'string') {
    throw new ValidationError('Message invalide', 'Le message doit être une chaîne de caractères');
  }
  ...
  if (trimmed.length > MAX_MESSAGE_LENGTH) {
    throw new ValidationError(
      'Message trop long',
      `Le message ne doit pas dépasser ${MAX_MESSAGE_LENGTH} caractères (${trimmed.length} actuellement)`
    );
  }
  ...
  if (xssPattern.test(trimmed)) {
    throw new ValidationError('Contenu non autorisé', 'Le message contient du code non autorisé');
  }
}
 ...
 */
function detectOffTopicLocal(message: string): OffTopicResult | null {
  ...
  const hasKeyword = RECYCLING_KEYWORDS.some((keyword: string) =>
 ...
 */
async function detectOffTopicGemini(message: string): Promise<OffTopicResult> {
  try {
    const model = getGeminiModel();
    const prompt = OFF_TOPIC_GEMINI_PROMPT.replace('{message}', message.replace(/"/g, "'"));
...
export async function askRecyclingAssistant(
  ...
  let authContext: AuthContext = { uid: 'anonymous', email: 'inconnu', authenticated: false };
  ...
  try {
    ...
    if (authToken) {
      ...
      throw new AuthError('Authentification requise', 'Token Bearer requis');
    }

    // ═══ 2. App Check ═══
    const appCheckValid = await verifyAppCheck(appCheckToken, authContext.uid);
    if (!appCheckValid && !process.env.FUNCTIONS_EMULATOR) {
      throw new AuthError('Vérification de sécurité échouée', 'AppCheck token invalide');
    }
    ...
    const timeoutPromise = new Promise<never>((_, reject) => {
      setTimeout(() => {
        reject(
          new TimeoutError('askAssistant', ASSISTANT_TIMEOUT_MS)
        );
    ...
    if (!text || text.trim().length === 0) {
      ...
      if (blockReason) {
        return {
          success: false,
          error: {
            code: ErrorCodes.GEMINI_SAFETY_BLOCKED,
    ...
    if (response.usageMetadata) {
      console.log(
        `[Assistant] Tokens utilisés: ${response.usageMetadata.totalTokenCount || 0}`
      );
    }

    const elapsed = Date.now() - startTime;
    console.log(
      `[Assistant] Succès | uid=${authContext.uid} | temps=${elapsed}ms`
    );
    ...
    if (errorMessage.includes('quota') || errorMessage.includes('429')) {
      return {
        success: false,
        error: {
          code: ErrorCodes.GEMINI_QUOTA_EXCEEDED,
