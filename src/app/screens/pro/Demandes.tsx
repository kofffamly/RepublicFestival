...
import { ChevronLeft, MapPin, Scale, Banknote, Clock, CheckCircle, X, Radio, ArrowRight, Loader, Bell, Sparkles, Navigation, Zap } from 'lucide-react';
...
function SuccessToast({ message, visible, onClose }: { message: string; visible: boolean; onClose: () => void }) {
  return (
    <AnimatePresence>
      {visible && (
        <motion.div
          ...
          className="absolute bottom-24 left-5 right-5 z-[100] pointer-events-auto"
        >
          <div
            className="flex items-center gap-3 px-5 py-3.5 rounded-2xl shadow-xl text-white"
            style={{
          ...
          >
            ...
            <div className="flex-1">
              <p className="text-xs font-black uppercase tracking-wider">Collecte acceptée !</p>
              <p className="text-[10px] text-white/80 mt-0.5">{message}</p>
            </div>
            <motion.button
              whileTap={{ scale: 0.9 }}
              onClick={onClose}
              className="w-7 h-7 rounded-full bg-white/20 flex items-center justify-center border-none cursor-pointer"
            >
              <X size={14} color="#fff" />
            </motion.button>
          </div>
...
function MiniMapRoute({ distance }: { distance: string }) {
  return (
    <div className="h-28 bg-slate-100 relative overflow-hidden border-b border-slate-100 flex items-center justify-center">
      ...
      <svg className="absolute inset-0 w-full h-full" viewBox="0 0 200 100">
        <motion.path
          d="M30 80 Q 60 20, 100 50 T 170 30"
          fill="none"
          stroke={GREEN}
          strokeWidth="2"
          strokeDasharray="6 4"
      ...
      <div className="absolute" style={{ left: '18%', top: '72%' }}>
        <motion.div
          className="w-4 h-4 rounded-full bg-orange-500 border-2 border-white shadow-md"
      ...
      <div className="absolute" style={{ left: '80%', top: '20%' }}>
        <motion.div
          className="w-4 h-4 rounded-full bg-emerald-500 border-2 border-white shadow-md"
          animate={{ scale: [1, 1.2, 1] }}
          transition={{ duration: 1.5, repeat: Infinity, delay: 0.5 }}
        />
      </div>

      <div className="absolute bottom-2 left-3 bg-slate-900/80 text-white font-black text-[9px] px-2 py-0.5 rounded-md flex items-center gap-1 z-10">
        <Navigation size={10} />
        <span>Itinéraire en direct: {distance}</span>
      </div>
    </div>
  );
}
  ...
  return (
    <MobileFrame bgColor="#F8F9F9">
      <div className="flex flex-col min-h-[760px] select-none relative">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-1.5 bg-orange-500/20 px-2.5 py-1 rounded-full border border-orange-500/30">
              ...
              <motion.span
                ...
                transition={{ duration: 1.5, repeat: Infinity }}
                className="text-[9px] text-orange-500 font-extrabold uppercase tracking-wider"
              >
                Flux Live
              </motion.span>
            </div>
        ...
        {/* Tab switcher with animated background and badge counters */}
          ...
          {(['pending', 'accepted'] as const).map(tab => {
            ...
            return (
              <button
                ...
                className="flex-1 py-3 text-xs font-black rounded-xl border-none cursor-pointer relative flex items-center justify-center gap-1.5"
                style={{
              ...
              >
                ...
                <span>{tab === 'pending' ? 'À collecter' : 'Acceptées'}</span>
        ...
        {/* Request List */}
        <div className="flex flex-col gap-4 px-5 pb-24 flex-1 overflow-y-auto">
              ...
              filtered.map((demande, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Interactive Simulated Map Grid — Enhanced */}
                    {demande.status === 'accepted' && (
                      <MiniMapRoute distance={demande.distance} />
                    )}
                    ...
                    <div className="p-4">
                      <div className="flex items-center gap-3 mb-4">
                        <motion.span
                          ...
                          className="text-3xl bg-slate-50 p-1 rounded-xl block"
                        >
                          {demande.emoji}
                        </motion.span>
                        <div>
                        ...
                        {index === 0 && demande.status === 'pending' && (
                          <motion.div
                            initial={{ scale: 0 }}
                            animate={{ scale: 1 }}
                            className="ml-auto flex items-center gap-1 bg-orange-500/10 px-2 py-1 rounded-lg"
                          >
                            <Sparkles size={10} color={ORANGE} />
                            <span className="text-[8px] font-black text-orange-500 uppercase tracking-wider">Urgent</span>
                          </motion.div>
                        )}
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-3 mb-4">
                        <motion.div
                          whileHover={{ y: -1 }}
                          className="flex items-center gap-2 p-3 bg-slate-50 border border-slate-100 rounded-2xl"
                        >
                        ...
                        {demande.status === 'pending' ? (
                          <div className="flex gap-3">
                            ...
                            <motion.button
                              ...
                              className="flex-[2] border-none cursor-pointer rounded-2xl py-3.5 flex items-center justify-center gap-1.5 text-white relative overflow-hidden"
                            ...
                            >
                              ...
                              {isAccepting ? (
                                <>
                                  ...
                                  <span className="text-xs font-black uppercase tracking-wider relative z-10">Confirmation...</span>
                                </>
                              ) : (
                                <>
                                  <CheckCircle size={16} color="#fff" />
                                  <span className="text-xs font-black uppercase tracking-wider relative z-10">Prendre la collecte</span>
                                </>
                              )}
                            </motion.button>
                          </div>
                          ...
                          <motion.div
                            ...
                            animate={{ opacity: 1, y: 0 }}
                            className="flex items-center justify-center gap-2 py-3 bg-emerald-500/10 rounded-2xl border border-emerald-500/15"
                          >
                            ...
                            >
                              <Navigation size={12} color={GREEN} />
                            </motion.div>
                          </motion.div>
                        )}
                      </div>
